        ...
        if tracks[0].boxes.id is not None:
            ...
            for box, track_id, cls in zip(boxes, track_ids, clss):
                ...
                if len(self.reg_pts) >= 3:
                    ...
                    if prev_position is not None and is_inside and not any([track_id, cls] == item[:2] for item in self.count_ids):
                        if track_id not in hashmap:
                            hashmap[track_id]=1
                            self.count_ids.append([track_id,cls,'source'])
                        else:
                            self.count_ids.append([track_id,cls,'dest'])
                ...
                # Count objects using line
                        ...
                        if distance < self.line_dist_thresh and not any([track_id, cls] == item[:2] for item in self.count_ids):
                            if track_id not in hashmap:
                                hashmap[track_id]=1
                                self.count_ids.append([track_id,cls,'source'])
                            else:

...
count_idJ1=counterJ1.count_ids
count_idJ2=counterJ2.count_ids
count_idJ3=counterJ3.count_ids
for count_id in count_idJ1.keys():
    data=count_idJ1[count_id]
    if(data[1]=='source'):
        if count_id in count_idJ2:
            vehicle_class_str=vehicle_classes[int(count_idJ2[count_id][0])]
            count_dict['BC'][vehicle_class_str]+=1
    
        if count_id in count_idJ3:
            vehicle_class_str=vehicle_classes[int(count_idJ3[count_id][0])]
            count_dict['BE'][vehicle_class_str]+=1

for count_id in count_idJ2.keys():
    data=count_idJ2[count_id]
    if(data[1]=='source'):
        if count_id in count_idJ1:
            vehicle_class_str=vehicle_classes[int(count_idJ1[count_id][0])]
            count_dict['DA'][vehicle_class_str]+=1
    
        if count_id in count_idJ3:
            vehicle_class_str=vehicle_classes[int(count_idJ3[count_id][0])]
            count_dict['DE'][vehicle_class_str]+=1
for count_id in count_idJ3.keys():
    data=count_idJ3[count_id]
    if(data[1]=='source'):
        if count_id in count_idJ1:
            vehicle_class_str=vehicle_classes[int(count_idJ1[count_id][0])]
...
with open('turning_patterns.csv', 'w', newline='') as csvfile:
    fieldnames = ['Turning Patterns'] + vehicle_classes
    writer = csv.DictWriter(csvfile, fieldnames=fieldnames)

    writer.writeheader()
    for pattern, counts in count_dict.items():
        row = {'Turning Patterns': pattern}
        row.update(counts)
        writer.writerow(row)
